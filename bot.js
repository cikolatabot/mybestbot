  ...
  const Jimp = require('jimp');
    ...
    ${videos.map(video2 => `**${++index} -** ${video2.title}`).join('\n')}
        ...
        **1-10 arasındaki arama sonuçlarından birini seçmek için lütfen bir değer belirtin. :warning: **
        ...
        if (!serverQueue) return msg.channel.send(':x: **Çalan müzik bulunmuyor**');
    ...
    **Şuanda Çalan Müzik:** ${serverQueue.songs[0].title}
        ...
        return msg.channel.send(':x: **Çalan müzik bulunmuyor.**');
    ...
    async function handleVideo(video, msg, voiceChannel, playlist = false) {
      ...
      if (!serverQueue) {
        ...
        try {
          ...
          console.error(`:x: **Odaya Giremiyorum: ${error}**`);
          queue.delete(msg.guild.id);
          return msg.channel.send(`:x: **Odaya Giremiyorum: ${error}**`);
        }
      } else {
